...
def QuaternionFromMatrix( RotMat ):
    ...
    norm = math.sqrt( sum( map( lambda x: x**2, Quaternion ) ) )
    Quaternion = map( lambda x: x/norm, Quaternion )
    return Quaternion
...
def AngleAxisFromQuaternion( Quaternion ):
    ...
    len = math.sqrt( sum( map( lambda x: x**2, imag ) ) )
    if len < sys.float_info.epsilon * 100:
        AngleAxis = imag;
    else:
        angle = 2 * math.atan2( len, real )
        AngleAxis = map( lambda x: angle/len * x, imag )
...
def RotationMatrixFromAngleAxis( k ):
    ...
    R = [[0]*3 for x in xrange(3)]
    fi = math.sqrt( sum( map( lambda x: x**2, k ) ) )
