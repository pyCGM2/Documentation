...
class NexusSegment( NexusObject.NexusObject ):
    ...
    def axisOrder( self, axisOrder ):
        ...
        if not isinstance( axisOrder, (str, unicode) ) or \
    ...
    def Write( self, name, SDK ):
        ...
        data = np.array( map( ConvertRotation, self._orientation.tolist() ), dtype=float  )
        data *= 180 / np.pi
    ...
    def Populate( self, origin, firstAxis, crossingAxis, axisOrder = None ):
        ...
        for i in xrange( np.shape( translation )[0] ):
            ...
            RotMat = map( list, zip( *[ col1, col2, col3 ] ) )
    ...
    def Position( self, frames=None ):
        '''Get the position of the segment origin. If ''frames'' is supplied, only positions at the requested frames are returned'''
    ...
    def SetPosition( self, components, frames=None ):
        ...
        num_frames = len( self._position )
        
        for i, index in enumerate( frameIndices ):
            self._translation[index] = component_array[i]
            
        for i in xrange( last_frame, num_frames ):
            np.append( self._translation, [[ np.nan, np.nan, np.nan ]], axis=0 )        

        
    def Orientation( self, frames=None ):
        '''Get the orientation of the segment. If ''frames'' is supplied, only orientations at the requested frames are returned'''
    ...
    def TranslatePointInSegment( self, point, translation, frames=None):
        '''Returns a NexusTrajectory whose position is ''point'' translated by ''translation'' in the segment local coordinate system'''
    ...
    def AngleBetween( self, segment, angle_type='euler', order='xyz' ):
        ...
        if not angle_type in ['fixed', 'euler', 'helical']:
        ...
        angle_output = map( ComputeAngle, rot1, rot2 )
